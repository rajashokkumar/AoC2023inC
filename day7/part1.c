...
char *cardName[] =  {"2", "3", "4", "5", "6", "7", "8", "9", "T", "J", "Q", "K", "A"};
...
int findHand(char *cards)
{
    ...
    if (threePair)
    {
        if(twoPair)
        {
            hand = 4;//Full house - one 3 match and one 2 match
        } else {
...
void addEntry(char *cards, int bid)
{
    ...
    while(temp)
    {
        ...
        } else if (new->hand == temp->hand)
        {
            for(i = 0; i < NUM_CARDS-1;i++)
            {
                //printf("[Curr]%c index %d , [Temp]%c index %d\n", cards[i], findCardIndex(cards[i]), temp->cards[i], findCardIndex(temp->cards[i]));
